const productPages = document.querySelectorAll('.product_page');

  productPages.forEach(function(productPage) {
    const productId = productPage.dataset.productId;
    const detailsButton = productPage.querySelector('.show-product-details');
    const productDesc = document.getElementById(`product-desc-KSh{productId}`);
    const closeButton = productDesc.querySelector(".close");
...
function formatCartItem(quantity, productName, productPrice, amount) {
  return `KSh{quantity.toString().padEnd(5)} KSh{productName.padEnd(20)} KSh{productPrice.padEnd(10)} KSh{amount.padEnd(10)}`;
}

const header = document.createElement("li");
header.textContent = formatCartItem("Qty", "Product Name", "Price(KSh)", "Amount(KSh)");
...
function updateTotals() {
  ...
  itemCostSpan.textContent = `KShKSh{itemTotal.toFixed(2)}`;
  taxSpan.textContent = `KShKSh{tax.toFixed(2)}`;
  shippingCostSpan.textContent = `KShKSh{shipping.toFixed(2)}`;
  totalCostSpan.textContent = `KShKSh{total.toFixed(2)}`;
}
